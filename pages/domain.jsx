...
const Domain = () => {
    ...
    const [domainDotCom, setDomainDotCom] = useState(0);
    const [domainDotNet, setDomainDotNet] = useState(0);
    const [domainDotOrg, setDomainDotOrg] = useState(0);
    const [tableValue, setTableValue] = useState([]);

    useEffect(() => {
        async function fetchData() {
            try {
                const nairaRes = await axios.get("https://backend.iruhost.com/api/get-naira");
                if (nairaRes.data.status !== "success") return;
                const value = nairaRes.data.value;
                setNairaValue(value);

                const priceRes = await axios.get("https://backend.iruhost.com/api/get-domain-prices");
                if (priceRes.data.status !== "success") return;

                const { dotCom, dotOrg, dotNet, result } = priceRes.data;
                setDomainDotCom(Number(dotCom));
                setDomainDotOrg(Number(dotOrg));
                setDomainDotNet(Number(dotNet));
    ...
    return ( 
        ...
        <section className="relative h-max w-screen bg-accent text-text">
            <div className="hidden sm:absolute -top-[60px] left-0 w-full h-max sm:flex flex-col sm:flex-row items-center justify-center gap-10">
                <div className="card bg-accent shadow-lg rounded p-4">
                    ...
                    <div className="h-max w-full">
                        <h2 className="text-base font-semibold">{locationDetail && locationDetail !== "NG" ? "$" : "₦"}{locationDetail && locationDetail !== "NG" ? Number(domainDotCom).toLocaleString() : Number(domainDotCom * nairaValue).toLocaleString()}/year</h2>
                    </div>
                </div>
                <div className="card bg-accent shadow-lg rounded p-4">
                    <div className="h-max w-full">
                    ...
                    <div className="h-max w-full">
                        <h2 className="text-base font-semibold">{locationDetail && locationDetail !== "NG" ? "$" : "₦"}{locationDetail && locationDetail !== "NG" ? Number(domainDotNet).toLocaleString() : Number(domainDotNet * nairaValue).toLocaleString()}/year</h2>
                    </div>
                </div>
                <div className="card bg-accent shadow-lg rounded p-4">
                    <div className="h-max w-full">
                    ...
                    <div className="h-max w-full">
                        <h2 className="text-base font-semibold">{locationDetail && locationDetail !== "NG" ? "$" : "₦"}{locationDetail && locationDetail !== "NG" ? Number(domainDotOrg).toLocaleString() : Number(domainDotOrg * nairaValue).toLocaleString()}/year</h2>
                    </div>
                </div>
            </div>
        ...
        <div className="w-full max-w-3xl overflow-x-auto">
                <table className="min-w-full border border-accent rounded shadow">
                    ...
                    <tbody>
                        {
                            tableValue.map((tdl, index) => {
                                const reg = Number(tdl.registration);
                                const ren = Number(tdl.renewal);
                                const tra = Number(tdl.transfer);
                                const currencySymbol = locationDetail && locationDetail !== "NG" ? "$" : "₦";
                                const isForeign = locationDetail && locationDetail !== "NG";
                                (
                                <tr key={index} className="tableVals">
                                    <td className="py-2 px-4 border-b">{tdl.tld}</td>
                                    <td className="py-2 px-4 border-b">{currencySymbol}{isForeign ? reg.toLocaleString(undefined, { minimumFractionDigits: 2 }) : Number(reg * nairaValue).toLocaleString()}</td>
                                    <td className="py-2 px-4 border-b">{currencySymbol}{isForeign ? ren.toLocaleString(undefined, { minimumFractionDigits: 2 }) : Number(ren * nairaValue).toLocaleString()}</td>
                                    <td className="py-2 px-4 border-b">{currencySymbol}{isForeign ? tra.toLocaleString(undefined, { minimumFractionDigits: 2 }) : Number(tra * nairaValue).toLocaleString()}</td>
                                </tr>
                            )})
                        }
                    </tbody>
